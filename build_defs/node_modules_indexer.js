...
`;
  ...
  for (const def of targetDefs) {
    ...
    const args = {
      ...
      module_root: `"${path.join('node_modules', def.name)}"`,
    };
    if (def.main) {
      args.entry_point = `"${def.main}"`;
    }
    ...
    if (link) {
      ...
      if (targets.indexOf(pkg) !== -1) {
        const args = {
          ...
          srcs: `[":${escapeName(pkg)}"]`,
          module_root: `"${path.join('node_modules', pkg)}"`,
          entry_point: `"${executableInPackage}"`
        };
...
function tracePackage(packageDir) {
  ...
  if (packageJson) {
    ...
    return {
      deps: deps,
      typings: typings,
      main: packageJson.main ? path.normalize(packageJson.main) : null
    };
  } else {
    // Assume no deps if package.json does not exist / is malformed
    return {deps: {}, typings: null, main: null};
  }
}
